...
chrome.runtime.onMessage.addListener(
  function(request, sender, sendResponse) {
    ...
    if(request.type === 'filter' && request.text === true){
    ...
    else if (request.type === 'translate' && request.text === true && translate){
      translate = false;
      $('body').on('mouseup',function(e){
        ...
        let selected = window.getSelection(),
            ...
            left = position.left+document.body.scrollLeft+document.documentElement.scrollLeft,//获取tips的left和top
            top = position.top+document.body.scrollTop+document.documentElement.scrollTop,
            width = position.width,
            height = position.height,
            random = parseInt(Math.random()*4),//上下左右取随机值
            pos = [
    ...
    }else if (request.type === 'translate' && request.text === false){
      translate = true;
      $('#tip').remove();
      $('body').off('mouseup');
      $('body').off('click');
